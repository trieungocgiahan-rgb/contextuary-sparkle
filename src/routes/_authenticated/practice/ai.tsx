...
} from "@/lib/ai-challenge.functions";
...
function AiChallengePage() {
  ...
  const gPassage = useServerFn(generatePassage);
  const gMisuse = useServerFn(generateMisusePair);
  const gEval = useServerFn(evaluateSentence);
